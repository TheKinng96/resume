# 職 務 経 歴 書

# 職務要約 
株式会社これからにてフルスタック開発エンジニアとして、広告管理プラットフォームの開発・最適化に約5年間携わっています。特にVue.js、TypeScript、Laravelを活用したWebアプリケーションの設計・開発を得意とし、Vue 2→3移行、AI機能実装、大規模ECプラットフォーム移行（30,000+商品）において大きな成果を挙げています。現在はチョク採向けATSの開発リードを担当。個人の副業ブランディング「Illumenza」を立ち上げ、「Illumenza Coupon」を公開（約30名利用中）、「Illumenza Points」を開発中。ビジュアルクリエイター向けプラットフォーム「Mostra.vip」もベータ運営中。React、Vue.js、TypeScript、Next.js、NestJS、Go等のフルスタック開発を専門としています。

## 活かせる経験・知識・技術
- 広告業界の業務知識
- TypeScriptとPHPによるWebアプリケーションの開発
- データベース(MongoDB、PostgreSQL)の設計
- API(REST)の設計
## アピールポイント
- 課題解決力が高く、複雑なプロジェクトを明確なマイルストーンに分割して効率よく管理できます。 
- AI技術を活用した機能提案・開発に積極的であり、ユーザーエクスペリエンスの大幅な向上を実現しました。 
- パフォーマンス最適化により、ユーザーの満足度とアプリケーションの応答速度を劇的に改善した実績があります。
## テクニカルスキル
## スキル一覧

### 言語

| 言語 | 使用期間 | レベル |
|------|----------|--------|
| JavaScript / TypeScript | 6年9ヶ月 | 状況に応じて最適なコードが書け、指導が可能 |
| PHP | 5年8ヶ月 | 状況に応じて最適なコードが書ける |
| HTML / CSS | 6年9ヶ月 | 状況に応じて最適なコードが書け、指導が可能 |
| Python | 1年0ヶ月（実務累計） | データ分析・処理の自動化が可能 |
| GoLang | 0年2ヶ月（実務） | 簡単なプログラミングが可能 |
| SwiftUI | 1年2ヶ月 | ある程度プログラミングが可能 |
| Java | 0年3ヶ月 | 簡単なプログラミングが可能 |

### フレームワーク

| フレームワーク | 使用期間 | レベル |
|----------------|----------|--------|
| VueJS | 5年8ヶ月 | 状況に応じて最適なコードが書け、指導が可能 |
| React | 6年9ヶ月 | 状況に応じて最適なコードが書け、指導が可能 |
| SvelteKit | 4年2ヶ月 | 状況に応じて最適なコードが書ける |
| WordPress | 5年8ヶ月 | 状況に応じて最適なコードが書ける |
| Laravel | 5年8ヶ月 | 状況に応じて最適なコードが書ける |
| NodeJS (ExpressJS, NestJS等) | 5年8ヶ月 | 状況に応じて最適なコードが書ける |
| Next.js | 0年3ヶ月 | 状況に応じて最適なコードが書ける |

### データベース

| DB | 使用期間 | レベル |
|----|----------|--------|
| MongoDB | 2年6ヶ月（実務累計） | Rawで操作可能 |
| PostgreSQL | 3年9ヶ月 | Rawで操作可能 |
| SQLite | 3年0ヶ月（実務累計） | Rawで操作可能 |

### その他

| 技術 | 使用期間 | レベル |
|------|----------|--------|
| Docker | 5年8ヶ月 | インストールから設定可能 |
| AWS | 6年5ヶ月（必要な部分のみ） | EC2, S3, Cloudfront, Lightsail, Route53, Amplify等操作可能 |


# 職務経歴詳細
## 株式会社これから（2020年8月〜現在）
事業内容:フルスタックWEBアプリ開発
従業員数:215名 資本金:1.1億円

### 【昇進履歴】 
- ジュニアフロントエンド開発者（2020年8月） 
- ミッドレベルフロントエンドエンジニア（2021年7月） 
- ジュニアフルスタックエンジニア（2022年2月） 
- ミッドレベルフルスタックエンジニア（2022年7月） 
- シニアソフトウェアエンジニア（2023年7月）

# 職務経歴

## プロジェクト一覧

| 期間 | プロジェクト名 | 項目 | 内容 |
|------|-------------|------|------|
| **2026年1月〜現在** | **チョク採 ATS開発** | **概要・成果** | 採用管理システム（ATS）のフルスタック開発・機能拡張<br>• 候補者パイプライン、求人管理、採用担当者ワークフロー機能を開発 |
| | | **技術スタック** | TypeScript, Next.js, NestJS, PostgreSQL |
| | | **規模・役割** | PJ全体: 2名 / **役割**: 技術リード・フルスタック開発 |
| **2025年6月〜現在** | **HubSpot ↔ Shopify 連携ツール開発** | **概要・成果** | セールスフロー自動化を目的とした双方向同期CLI/UI Extension開発<br>• 開発開始1週間でMVP完成<br>• UI Extensionを3日でプロトタイプ |
| | | **技術スタック** | TypeScript, React (HubSpot UI Extension SDK), Node.js (Shopify Admin GraphQL API), Docker |
| | | **規模・役割** | PJ全体: 1名 / **役割**: フルサイクル開発 |
| **2025年4月〜6月** | **楽天市場 → Shopify 移行CLIツール** | **概要・成果** | 約40,000件のCSVデータ一括移行ツール開発<br>• 処理時間を2時間→20分へ短縮(メモリ使用量80%削減)<br>• 予定より2週間早期リリース |
| | | **技術スタック** | Python (pandas, csv, requests), Google Apps Script |
| | | **規模・役割** | PJ全体: 1名 / **役割**: フルサイクル開発 |
| **2025年3月〜現在** | **チョク採サポート開発** | **概要・成果** | 新規開発プロジェクトのバックログ対応<br>• 不具合再発率の低減に貢献<br>• ナレッジ共有の仕組み化で開発効率向上 |
| | | **技術スタック** | TypeScript, PHP, Vue.js, Laravel, PostgreSQL, Retool |
| | | **規模・役割** | PJ全体: 6名 / **役割**: サポート開発 |
| **2025年3月〜4月** | **セールスデータ処理ツール** | **概要・成果** | 営業データの集計・加工自動化ツール開発(2社向け)<br>• データ処理工数を80%削減<br>• 計画通りのスケジュールで完了 |
| | | **技術スタック** | Google Apps Script (JavaScript) |
| | | **規模・役割** | PJ全体: 1名 / **役割**: 基本設計〜運用保守 |
| **2023年10月〜2025年3月** | **BuzzEC/Buzzify/チョク採<br>リプレイスプロジェクト** | **概要・成果** | 既存AdSISTアプリのクローン・カスタマイズ開発<br>• 累計ユーザー数2万人突破<br>• 重大障害ゼロでリリース<br>• TikTok広告導入、One Pixel機能実装 |
| | | **技術スタック** | TypeScript, PHP, Vue.js, Laravel, PostgreSQL, BitBucket Pipeline, Retool |
| | | **規模・役割** | PJ全体: 14名 / **役割**: UI開発責任者 |
| **2023年3月〜7月** | **UI全面刷新・Vue3移行** | **概要・成果** | アプリUI再設計とVue3アップデート<br>• 予定5ヶ月を3.5ヶ月で完了<br>• 顧客問い合わせ数減少、ロード時間短縮 |
| | | **技術スタック** | TypeScript, PHP, Vue.js, Laravel, PostgreSQL, MongoDB, BitBucket Pipeline |
| | | **規模・役割** | PJ全体: 3名 / **役割**: PJ責任者・進捗管理・UI開発 |
| **2022年12月〜2023年1月** | **AI機能統合** | **概要・成果** | ChatGPTを活用した広告テキスト生成機能開発<br>• 90%以上の広告が初期テキスト生成にAI利用<br>• URL分析による自動コンテンツ提案機能 |
| | | **技術スタック** | TypeScript, PHP, Vue.js, Laravel, PostgreSQL, MongoDB, BitBucket Pipeline |
| | | **規模・役割** | PJ全体: 1名 / **役割**: 提案〜実装 |
| **2022年5月〜7月** | **WordPressプラグイン開発** | **概要・成果** | AdSISTアプリのWordPressプラグイン開発<br>• リリース直後に約20名の新規ユーザー獲得<br>• WordPress経由での導入促進 |
| | | **技術スタック** | PHP, JavaScript, WordPress, Laravel, MongoDB |
| | | **規模・役割** | PJ全体: 2名 / **役割**: UI開発・タグ設定フロー |
| **2022年1月〜3月** | **LINE広告配信サービス導入** | **概要・成果** | 4プラットフォームでのLINE広告配信機能構築<br>• LINE広告経由流入が全体の約20%まで成長<br>• ランディングページ制作 |
| | | **技術スタック** | Vue.js, Laravel, MongoDB, BitBucket Pipeline |
| | | **規模・役割** | PJ全体: 3名 / **役割**: UI開発・広告フロー・LP制作 |
| **2021年6月〜12月** | **Google広告配信サービス導入** | **概要・成果** | 4プラットフォームでのGoogle広告配信機能実装<br>• 月間ユーザー数ピーク更新<br>• 累計ユーザー数1万人突破 |
| | | **技術スタック** | Vue.js, Laravel, MongoDB, BitBucket Pipeline |
| | | **規模・役割** | PJ全体: 4名 / **役割**: UI開発・LP制作 |
| **2020年12月〜2021年3月** | **ランディングページ更新** | **概要・成果** | Figmaデザインベースの3つのLP更新<br>• ページ経由ユーザー数6,000人突破<br>• 各メディアで紹介される |
| | | **技術スタック** | WordPress, AWS Lightsail |
| | | **規模・役割** | PJ全体: 1名 / **役割**: サイト開発・サーバー設置 |
| **2020年9月〜11月** | **Yahoo広告配信サービス導入** | **概要・成果** | AdSISTアプリにYahoo広告配信機能組み込み<br>• 新規流入チャネル構築<br>• チャットボットフロー設計 |
| | | **技術スタック** | Vue.js, Laravel, MongoDB, BitBucket Pipeline |
| | | **規模・役割** | PJ全体: 4名 / **役割**: UI開発 |

## 主な実績サマリー

### 🚀 パフォーマンス改善
- ページロード時間: **10秒 → 500ms** (95%改善)
- データ処理時間: **2時間 → 20分** (83%短縮)
- メモリ使用量: **80%削減**

### 📈 ビジネスインパクト
- 累計ユーザー数: **2万人突破**
- AI機能利用率: **90%以上**
- LINE広告流入: **全体の20%**
- データ処理工数: **80%削減**

### ⚡ 開発効率
- MVP開発: **1週間**
- UI Extension プロトタイプ: **3日**
- 予定5ヶ月のプロジェクトを **3.5ヶ月で完了**

# 副業・サイドプロジェクト

## Illumenza Coupon（2024年12月〜）
[illumenza.dev](https://illumenza.dev) | [Illumenza Coupon](https://coupon.illumenza.dev) | [デモストア](https://32mby6vgei.shop-pro.jp/)

**ステータス**: 公開済み・約30名のユーザーが利用中  
**事業形態**: 個人事業主として登録

**サービス内容**:
- スピンホイール形式のインタラクティブな顧客データ収集システム
- 事業者サイトにマッチするカスタマイズ可能なテーマ設定
- 収集データのエクスポート機能と基本的な分析ダッシュボード
- カラーミープラットフォームとのOAuth連携実装

**技術スタック**:
- フロントエンド: SvelteKit（Vercel）
- バックエンド: NestJS（自社VPS・Docker / Redis / PostgreSQL）
- API連携: カラーミーOAuth 2.0、スクリプトタグ埋め込み（Cloudflare R2で配信）

## Illumenza Points（2025年〜）

**ステータス**: 開発中（カラーミー向け）

**サービス内容**:
- カラーミー事業者向けポイント型ロイヤルティアプリ
- 購入でポイント付与・特典交換のルール設定が可能
- カラーミーOAuth・スクリプトタグ連携

**技術スタック**: SvelteKit（Vercel）、NestJS（自社VPS・Docker / Redis / PostgreSQL）、カラーミーAPI、Cloudflare R2

## Mostra.vip（2025年〜）
[mostra.vip](https://mostra.vip)

**ステータス**: ベータ版・招待制

**サービス内容**:
- ビジュアルコンテンツクリエイター向けプラットフォーム
- アップロードした画像をフル解像度で保存・展示（SNSのような圧縮なし）
- 画像は保護されており、ダウンロードはオーナーのみ可能

**技術スタック**: SvelteKit（Vercel）、Goバックエンド（自社VPS・Docker / Redis / PostgreSQL）

# 自己PR
技術的課題の迅速な解決や新技術への積極的な挑戦を心がけています。個人開発者としてプロジェクトを独立して遂行しながら、営業チームやクライアントとの効果的なコラボレーションを実現し、ユーザーフレンドリーなシステム開発に貢献していきたいと考えています。


